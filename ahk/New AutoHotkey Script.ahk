F10::
FileRead, Bitmap, *c %A_ScriptDir%\testCursor.bmp
hBitmap := CreateBitmap(Bitmap)	;, 200, 150)
Gui, Margin, 20, 20
Gui, Add, Picture, w200 h150 0xE hWndPic1		
SendMessage, 0x172, 0x0, hBitmap, , ahk_id %Pic1%
Gui, Show
Return

CreateBitmap(BitmapBuffer) {
	Return DllCall( "CreateDIBitmap"
		, Ptr, DllCall("GetDC", Ptr, A_ScriptHwnd)	;hDC
		, UInt, (bmiHAddr:=&BitmapBuffer+14)
		, UInt, (CBM_INIT:=4)
		, UInt, &BitmapBuffer+NumGet(BitmapBuffer,10)
		, UInt, &BitmapBuffer+14
...
CaptureCursor(captureTo="clipboard", cursorSize=32)
{
	...
	;Gdip_SaveBitmapToFile(hBM, "Test.png")
